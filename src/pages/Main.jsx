...
export default function Main() {
  ...
  const handleMovieClick = (movieId) => {
    ...
    console.log(
      "Сохраняем состояние при переходе на детальную страницу:",
      currentState,
    );
  ...
  const applyAllFilters = (filters) => {
    ...
    // Проверяем, пустые ли все фильтры
    ...
    // Получаем текущее значение таба из состояния
    const currentTab = activeTab;

    // Применяем фильтры только если есть ID пользователя
    if (userIdInput) {
      if (areFiltersEmpty) {
        // Если все фильтры пустые, получаем нефильтрованные рекомендации
        console.log(
        ...
        // Создаем параметры запроса напрямую здесь, чтобы избежать проблем с асинхронным обновлением состояния
  ...
  const clearAllFilters = () => {
    ...
    // Получаем текущее значение таба до изменения состояния
    const currentTab = activeTab;

    // Если есть ID пользователя, обновляем результаты без фильтров
    if (userIdInput) {
      if (currentTab === "popular") {
        fetchSimilarUsersRecommendations(userIdInput);
      } else {
  ...
  const applyFilters = (currentTab = null) => {
    ...
    // Используем переданное значение таба, если оно есть, иначе используем состояние activeTab
